...
static int sf_mmap(struct file *filp, struct vm_area_struct *vma);

/* the file operations, i.e. all character device methods */
static struct file_operations sf_fops = {
        .open = sf_open,
        .release = sf_release,
        .mmap = sf_mmap,
	.unlocked_ioctl = sf_ioctl,
        .owner = THIS_MODULE,
};

/* the character device context structure */
static struct sf_context dev_context;
...
static int __init igb_init_module(void)
{
	...
	ret = pci_register_driver(&igb_driver);
        ...
        if ((ret = alloc_chrdev_region(&sf_dev, 0, 1, "mmap")) < 0) {
                printk(KERN_ERR "could not allocate major number for mmap\n");
                goto out;
        }

        /* initialize the device structure and register the device with the kernel */
        cdev_init(&sf_cdev, &sf_fops);
        if ((ret = cdev_add(&mmap_cdev, sf_dev, 1)) < 0) {
                printk(KERN_ERR "could not allocate chrdev for mmap\n");
                goto out_unalloc_region;
        }

        /* zero out the device context... */
	    memset(&dev_context, 0, sizeof(struct sf_context));
	    init_MUTEX(&dev_context.sem);
	    init_waitqueue_head(&dev_context.wq);
#endif
...
#ifdef _SOMBERFRAMES
    /*
     * we need to clear up tha mmapped area!
     *

    struct vma_area_struct * vma = NULL;
    struct mm_struct *mm = current->mm;

    for(i=0 ; i<dev_context.num_bufs ; i++)
    {
        dev_context.buf_refcnt[i] = 1;
        vma = find_vma(mm, dev_context.ubufs[i]);
        sf_vma_close(vma);
    }*/

    /* remove the character deivce */
    cdev_del(&sf_cdev);
    unregister_chrdev_region(sf_dev, 1);
#endif
...
struct igb_adapter *adapters[MAX_ADAPTERS];
#endif
...
static int sf_release(struct inode *inode, struct file *filp)
{
    ...
    struct sf_context *context = filp->private_data;

	if (!context)
		return 0;

    /* we should free up mmapped buffers in use... */
    context->fds--;
...
void sf_vma_open(struct vm_area_struct *vma)
{
	struct file *filp = vma->vm_file;
	struct sf_context *context;
	int i;
...
void sf_vma_close(struct vm_area_struct *vma)
{
	struct file *filp = vma->vm_file;
	struct sf_context *context;
	int i;
...
static struct vm_operations_struct sf_vm_ops = {
	.open = sf_vma_open,
	.close = sf_vma_close,
};
...
static int sf_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (!context) {
		printk("sf_mmap() failed: context == NULL\n");
		return -EINVAL;
	}

	if (context->num_bufs >= MAX_BUFS) {
		printk("sf_mmap() failed: exceeding %d bufs\n", MAX_BUFS);
		return -ENOMEM;
	}

	/* chunk size is (# of packets x packet size). */
	size = PAGE_ALIGN(MAX_PACKET_SIZE * MAX_CHUNK_SIZE);
	...
	if (vma->vm_end - vma->vm_start != size) {
		printk("sf_mmap() failed: invalid size\n");
		return -EINVAL;
	}

	if (vma->vm_pgoff) {
		printk("sf_mmap() failed: vma->vm_pgoff != 0\n");
		return -EINVAL;
	}
	...
	/* Clear this up if we run into problems while reserving pages. */
	if (err) {
		for (kp = buf; size > 0; kp += PAGE_SIZE, size -= PAGE_SIZE)
			ClearPageReserved(vmalloc_to_page(kp));
		vfree(buf);
		printk(KERN_ERR "sf_mmap() failed! err=%d\n", err);
	} else  {
...
int sf_list_devices(struct sf_device __user *devices_usr)
{
	struct sf_device *devices;
	int i;
	int n;

	devices = kmalloc(sizeof(struct sf_device) * MAX_DEVICES, GFP_USER);
	if (!devices) {
	...
	for (i = 0; i < n; i++) 
	{
		struct igb_adapter *adapter = adapters[i];
		struct net_device *netdev = adapter->netdev;
...
int sf_attach_rx_device(struct sf_context *context,
		struct sf_device __user *device_usr)
{
	struct sf_device device;
	struct igb_adapter *adapter;

	if (copy_from_user(&device, device_usr, sizeof(struct sf_device)))
		return -EFAULT;

	if (device.ifindex < 0 || device.ifindex >= adapters_found) {
		printk(KERN_ERR "sf_attach_rx_device(): invalid ifindex\n");
		return -EINVAL;
	}

	adapter = adapters[device.ifindex];
...
int sf_detach_rx_device(struct sf_context *context,
		struct sf_device __user *device_usr)
{
	struct sf_device device;
	struct igb_adapter *adapter;
...
int sf_copy_rx_packets(struct igb_adapter *adapter,
		int n,
...
{
	int i;
	struct igb_ring *rx_ring = NULL;
	struct igb_q_vector *q_vector = NULL;
	union e1000_adv_rx_desc *rx_desc;
#ifndef CONFIG_IGB_DISABLE_PACKET_SPLIT
	const int current_node = numa_node_id();
#endif
	...
	/* we go through all q_vectors... */
	for(i = 1 ; i < adapter->num_q_vectors && cnt < n ; i++)
	{
		q_vector = &adapter->q_vector[i];
		rx_ring = q_vector->rx.ring;
		next = rx_ring->next_to_clean;
		rx_desc = IGB_RX_DESC(rx_ring, next);
#ifdef IGB_DCA
		if (q_vector->adapter->flags & IGB_FLAG_DCA_ENABLED)
			igb_update_dca(q_vector);
#endif
		...
		while (igb_test_staterr(rx_desc, E1000_RXD_STAT_DD) && cnt<n ) 
		{
			struct igb_rx_buffer *buffer_info = &rx_ring->rx_buffer_info[next];
			struct sk_buff *skb = buffer_info->skb;
			union e1000_adv_rx_desc *next_rxd;
			...
			next_rxd = IGB_RX_DESC(rx_ring, next);
			prefetch(next_rxd);
			...
			{
				struct igb_rx_buffer *next_buffer;
				next_buffer = &rx_ring->rx_buffer_info[next];
				buffer_info->skb = next_buffer->skb;
				buffer_info->dma = next_buffer->dma;
				next_buffer->skb = skb;
				next_buffer->dma = 0;
			...
			/* Hand that SKB to the MMAP */

			/* 
			 * when there is paged data, the packet begins at skb->data for 
			 * skb_headlen(skb) bytes, then continues on into the paged data 
			 * area for skb->data_len bytes. That is why it is illogical to 
			 * try and do an skb_put(skb) when there is paged data. You have 
			 * to add data onto the end of the paged data area instead. 
			 *
			 * */

			skb_copy_bits(skb, 0, pkt_buf + sf_offset, 
					skb_headlen(skb) + skb->data_len);

			info[cnt].offset = sf_offset;
			info[cnt].len = skb_headlen(skb) + skb->data_len;
			sf_offset = ALIGN(sf_offset + skb_headlen(skb) + skb->data_len, 64);
...
int sf_recv_chunk(struct sf_context *context, struct sf_chunk __user *chunk_usr)
{
	struct sf_chunk chunk;
	struct igb_adapter *adapter = NULL;
	...
	if (!access_ok(VERIFY_WRITE, chunk.info, 
			chunk.cnt * sizeof(struct sf_pkt_info)))
		return -EFAULT;

	/* we find the kernel buffer associated */
	for (i = 0; i < context->num_bufs; i++)
	...
	/* we need to figure the waiting out... */
retry:
	if (chunk.recv_blocking)
		prepare_to_wait(&context->wq, &wait, TASK_INTERRUPTIBLE);

	ret = sf_copy_rx_packets(context->adapter, 
			chunk.cnt, 
			context->info,
			kbuf, 
			offset);
	
	/* some packets were copied... */
	if (ret) {
		if (chunk.recv_blocking)
	...
	/* 
	 * In the original packet shader implementation, interrupts
	 * were temporarily enabled... I am trying to understand why
	 * really. If we're polling we're polling, why enable 
	 * temporarily the interrupts again.
	 *
	 *
...
found:
	...
	*/
	...
	put_user(context->adapter->bd_number, &chunk_usr->queue.ifindex);
	return ret;
}
